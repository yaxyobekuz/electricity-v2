...
/**
 * Tab 2 — hozirgi sozlamalar va ma'lumotlar: filtrlar, TP jadvali,
 * yangi TP qo'shish formasi.
 */
export async function TpListView({
  searchParams,
}: {
  searchParams: { feeder?: string; q?: string };
}) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {/* Sarlavha sahifa qobig'ida — bu yerda faqat filtrlar. */}
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="text-sm text-zinc-500">
          {tpPoints.length} ta transformator punkti
          {(query || feederId) && " (filtrlangan)"}
        </p>
        <TpFilters feeders={feeders} feeder={feederId ?? "all"} query={query} />
      </div>
